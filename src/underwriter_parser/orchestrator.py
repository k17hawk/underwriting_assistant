...
class Orchestrator:
    ...
    def __init__(self):
        ...
        # Initialize MongoDB storage (uses certifi connection)
    ...
    def generate_correlation_id(self) -> str:
        """Generate unique correlation ID: timestamp + UUID4 segment."""
    ...
    def save_file_to_artifact(self, correlation_id: str, file_content: bytes, 
                             filename: str = "original.pdf") -> Path:
    ...
    def send_to_kafka(self, correlation_id: str, file_path: Path, 
                     retry_count: int = 0, original_filename: str = None):
    ...
    def process_submission(self, file_content: bytes, 
        ...
        with self.tracer.start_as_current_span("orchestrator.process") as span:
            ...
            # Save PDF to local artifact
            file_path = self.save_file_to_artifact(correlation_id, processed_content, original_filename)
            print(f"💾 Saved PDF to: {file_path}")
            
            # Create MongoDB record - THIS IS WHERE DATA GOES TO MONGODB
